...
import './Articles.css'
import GuidesCover  from '../../../assets/GuidesCover.jpg'
...
const Guides = () => {
  ...
  useEffect(() => {
    const fetchResources = async () => {
        try {
          setLoadingHome(true)
            const res = await axios.get(`http://localhost:8080/api/v1/user/resources/guide`, {
                headers: {
            ...
            if (res.data.success) {
                ...
                const transformedResources = res.data.data.map(article => [
                    article._id,
                    article.title,
                    article.description,
                    article.link,
...
const readHandle=async (title)=>{
  ...
  return (
    <Template Cover={GuidesCover} Head={'Guides'} Children={
      <>
      ...
      <form class="max-w-full w-3/4 mx-auto">   
          ...
          <div class="relative">
              ...
              <input type="search" id="default-search" class="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value={serachInput} onChange={(e)=>{setSearchInput(e.target.value)}} placeholder="Search" required />
